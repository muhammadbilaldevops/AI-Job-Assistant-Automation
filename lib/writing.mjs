...
export async function generate(db, uid, jobId, kind) {
  ...
  if (!profile.confirmed || !profile.skills?.trim())
    throw Error(
      "Confirm your profile and add your real skills before creating a document.",
    );
  if (kind === "resume" && !profile.experience?.trim())
    throw Error(
      "Your exact resume preset requires one real experience entry. Add a real internship, employment or clearly labelled project experience with its true title and dates.",
    );
  ...
  if (errors.length)
    throw Error(
      "The draft did not pass your writing rules: " +
